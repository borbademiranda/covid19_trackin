---
title: 'Covid-19 no Brasil'
date: "`r format(Sys.time(), '%d %B, %Y')`"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r packages, include=FALSE}
library(tidyverse)
library(leaflet)
library(readxl)
library(RCurl)
library(stringr)
library(htmltools)
library(rgdal)
library(shiny)
library(plotly)
library(lubridate)
library(kableExtra)
library(data.table)
```


```{r covidbr, include=FALSE}
# arquivo temporário para baixar dados
temp <- tempfile()

# braixando dados
download.file("https://data.brasil.io/dataset/covid19/caso.csv.gz", temp)

brazil <- as.data.frame(data.table::fread('https://www.brasil.io/dataset/covid19/caso?format=csv'))

# importando
#brazil <- read.csv('https://data.brasil.io/dataset/covid19/caso.csv.gz',
 #                  sep = ",", header = T, stringsAsFactors = F, encoding = 'UTF-8')
names(brazil)

table(brazil$is_last)

brazil$is_last <- as.logical(brazil$is_last)
brazil$date <- as.Date(brazil$date)
brazil$date_2 <- as.Date(brazil$date) %>% format('%d/%m/%Y')

```


```{r geodata, include=FALSE, warning=FALSE}
# coordenadas dos municípios brasileiros
mun <- read.csv('C:/Users/borba/OneDrive/Área de Trabalho/2020/covid_tracking/data/mun_coord.csv', encoding = 'UTF-8')

mun$city <- mun$NOME_MUNICIPIO

# transformando os nomes dos municípios para caixa alta
brazil$city <- toupper(brazil$city)

# mergindo dados
bd <- merge(mun, brazil %>% filter(is_last == T), by = 'city')

# ajustando o conteúdo da variável a ser plotada
cidade <- paste('Cidade:', bd$city)
confirmados <- paste('Casos confirmados:', bd$confirmed)
mortos <- paste('Mortos:', bd$deaths)
taxa_100k <- paste('Infectados/100 mil habitantes', bd$confirmed_per_100k_inhabitants)
mortalidade <- paste('Taxa de mortalidade', bd$death_rate)
atualizado <- paste('Atualizado em:', bd$date_2)

carai <- paste(cidade, '<br/>',
               confirmados, '<br/>',
               mortos, '<br/>',
               taxa_100k, '<br/>',
               mortalidade, '<br/>', 
               atualizado, sep = '') %>%
  lapply(HTML)
summary(bd$confirmed_per_100k_inhabitants)

mybins <- c(seq(0, 30, 6), seq(50, 1000, 250))

# criando paleta de cores
palette <- colorNumeric(palette="Reds", domain = NULL, na.color="transparent")

palette2 <- colorBin(palette="Reds", bd$confirmed_per_100k_inhabitants,  mybins, pretty = F)

#shape <- readOGR('../../../../borba/OneDrive/Área de Trabalho/data/shapefiles/br/BRMUE250GC_SIR.shp',
 #                GDAL1_integer64_policy = T)
```

O total de casos registrados no Brasil até o dia `r format(Sys.Date(), '%d/%m/%Y')` é de `r sum(bd$confirmed)`. Podemos observar no mapa abaixo algumas estatísticas sobre a Covid-19 nos municípios brasileiros. Os dados são atualizados diariamente, dependendo das secretarias estaduais de saúde, tendo em vista que os dados são produzidos a nível municipal. 

```{r map, echo=FALSE, fig.align=TRUE, warning=FALSE}

m <- bd %>%
  leaflet() %>% 
  addTiles() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  clearMarkers() %>%
      clearShapes() %>%
  #addPolygons(data = shape,
   #           weight = 0, smoothFactor = 0.5, opacity = .5, 
    #          fillOpacity = 0.5, label = carai,
    #fillColor = ~palette(bd$confirmed),
    #highlightOptions = highlightOptions(color = "white", weight = 1, 
     #                                   bringToFront = TRUE),
    #labelOptions = labelOptions(style = list("font-weight" = "normal", 
     #                                        padding = "3px 8px"), 
      #                          textsize = "13px", direction = "auto")) 
  addCircleMarkers(lng = ~LONGITUDE, lat = ~LATITUDE,
                   radius = ~sqrt(bd$confirmed_per_100k_inhabitants), stroke = FALSE, fillOpacity = 0.5,
                   label = carai, color = ~palette2(bd$confirmed_per_100k_inhabitants),
                   labelOptions = labelOptions(style = list("font-weight" = "normal", 
                                                            padding = "3px 8px"), 
                                               textsize = "13px", 
                                               direction = "auto")) %>%
   addLegend(pal = palette2, values = ~confirmed, opacity=0.9, 
             title = "Confirmados/100 mil habitantes", position = "bottomright" )


m

```

```{r data, include=FALSE}

# filtering states data
states <- brazil %>% 
  filter(place_type == 'state') %>%
  select(-starts_with('city'))# %>%
  #group_by(state) %>%
  #summarize('confirmed' = sum(confirmed, na.rm = T),
   #         'deaths' = sum(deaths, na.rm = T),
    #        'death_rate' = sum(death_rate, na.rm = T))

```


Na tabela abaixo vemos a distribuição de algumas variáveis por UF no Brasil. Verificamos que São Paulo é o estado com maior número de infectados (`r states %>% filter(place_type == 'state' & is_last == T & state == 'SP') %>% select(confirmed) %>% sum()`), seguido pelo Rio de Janeiro (`r states %>% filter(place_type == 'state' & is_last == T & state == 'RJ') %>% select(confirmed) %>% sum()`) e Ceará (`r states %>% filter(place_type == 'state' & is_last == T & state == 'CE') %>% select(confirmed) %>% sum()`). Já no que se refere ao número de mortes, os primeiros colocados são São Paulo, Rio de Janeiro e Recife, com `r print(states$confirmed[states$state == 'SP' & states$is_last == T])`, `r print(states$confirmed[states$state == 'RJ' & states$is_last == T])` e `r print(states$confirmed[states$state == 'SP' & states$is_last == T])`. 

```{r table, echo=FALSE}
states[order(states$state),] %>%
  filter(place_type == 'state' & is_last == T) %>%
  select(state, confirmed, deaths, death_rate) %>%
  #group_by(state) %>%
  #summarize('Estado' = sum(deaths, na.rm = T),
   #         'Confirmados', sum(confirmed, na.rm = T),
    #        'Taxa de mortalidade' = sum(death_rate, na.rm = T)) %>%
  dplyr::transmute('Estado' = state,
                   'Casos confirmados' = confirmed,
                   'Mortes' = deaths,
                   'Taxa de mortalidade' = death_rate) %>%
  kable() %>%
  kable_styling() %>%
  footnote(general = paste('Dados atualizados em:', format(Sys.Date(), '%d/%m/%Y'), 'às', str_sub(Sys.time(), -9)))
```

Se analisarmos os estados com maior taxa de mortalidade (Mortos/Infectados), verificamos que Paraíba, Piauí e Pernambuco são os estados com as maiores taxas de mortalidade no Brasil.

```{r tab-2, echo=FALSE}
states[order(desc(states$death_rate)),] %>% filter(place_type == 'state' & is_last == T) %>% dplyr::select(state, death_rate) %>% head(3) %>%
  dplyr::transmute('Estado' = state,
                   'Taxa de Mortalidade' = death_rate) %>%
  kable() %>%
  kable_styling(full_width = F) %>%
   footnote(general = paste('Dados atualizados em:', format(Sys.Date(), '%d/%m/%Y'), 'às', str_sub(Sys.time(), -9)))
```


Podemos ver nos gráficos abaixo o acumulado de casos por estado ao longo do tempo. 

```{r plots, echo=FALSE}
knitr::include_app('https://lucasbmiranda.shinyapps.io/ts_brstates/', height = 6)
#for (i in seq_along(states$state)) {
#  plot <- ggplot(subset(states, states$state == states_list[i]),
#                 aes(date, confirmed, group = state)) +
#    geom_line()
#  print(plot)
#}


#inputPanel(
#
#  selectInput('breaks2', label = 'Estados:',
#  choices = distinct(states, state)),
#  
#  #checkboxGroupInput('breaks2', label = 'Estados:', 
#   #                  choices = unique(states$state), 
#    #                 selected = unique(states$state)[1]),
#  
#  checkboxInput('log_date', label = 'Escala logarítmica', FALSE)
  
#)

#renderPlotly({
 
   
#    l1 <- ggplot(states %>% filter(state == input$breaks2), 
#       aes(x = date, y = confirmed)) +
#  geom_line() +
#  labs(title = paste("Total de casos", input$breaks2),
#       x = "", 
#       y = "Confirmados")+
  #guides(color=guide_legend(title="Países"))+
#  theme_minimal()+
#  theme(plot.title = element_text(face = "bold", size = 12, hjust = 0.5),
#        plot.subtitle = element_text(face = "italic", size = 12, hjust = 0.5)) +
#      scale_x_date(date_breaks = '1 week')
  
#  if (input$log_date)
    
     
   
#    l1 <- ggplot(states %>% filter(state == input$breaks2), 
#       aes(x = date, y = log(confirmed))) +
#  geom_line() +
#  labs(title = paste("Total de casos", input$breaks2),
#       x = "", 
#       y = "Confirmados (log)")+
  #guides(color=guide_legend(title="Países"))+
#  theme_minimal()+
#  theme(plot.title = element_text(face = "bold", size = 12, hjust = 0.5),
#        plot.subtitle = element_text(face = "italic", size = 12, hjust = 0.5)) +
#      scale_x_date(date_breaks = '1 week')
  
#  return(l1)

#})
```



